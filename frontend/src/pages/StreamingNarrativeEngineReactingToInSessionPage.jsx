...
export default function StreamingNarrativeEngineReactingToInSessionPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/cf-streaming-narrative-engine-reacting-to-in-session-/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Streaming narrative engine reacting to in-session events with cinematic descriptions</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-streaming-narrative-engine-reacting-to-in-session-/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
