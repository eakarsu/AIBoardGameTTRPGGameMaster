...
export default function NoVirtualTabletopVttInteractiveBoardPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/gap-no-virtual-tabletop-vtt-interactive-board/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>No virtual tabletop (VTT) interactive board</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-no-virtual-tabletop-vtt-interactive-board/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
