...
export default function AgenticGmCoPilotThatRunsEncountersTracksToPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/cf-agentic-gm-co-pilot-that-runs-encounters-tracks-to/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Agentic GM co-pilot that runs encounters, tracks tone, and adapts to player choices in real time</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-agentic-gm-co-pilot-that-runs-encounters-tracks-to/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
