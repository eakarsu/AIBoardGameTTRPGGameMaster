...
export default function RagOverRulebooksDD5ePathfinderEtcForInstaPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/cf-rag-over-rulebooks-d-d-5e-pathfinder-etc-for-insta/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>RAG over rulebooks (D&D 5e, Pathfinder, etc.) for instant ruling answers with citations</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-rag-over-rulebooks-d-d-5e-pathfinder-etc-for-insta/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
